...
/* Subtle, slow, continuous rotation of ONLY the dashed/dots ring in the logo
   mark (the play triangle + gold tile stay static). Applies wherever the mark
   renders with this shared CSS: Viewer + Team headers and the gate's logo. */
.logo-ring{transform-box:fill-box;transform-origin:center;animation:crafted-ring-spin 11s linear infinite}
@keyframes crafted-ring-spin{to{transform:rotate(360deg)}}
...
def render_header(ts, brand_href=None):
    ...
    The Viewer is strictly read-only, so there is no "Register New Video"
    call-to-action here — registration lives only behind the Team gate.
    """
...
def render_footer(switch_href="index.html"):
    """Page footer. No external links anywhere (the GitHub link was removed).

    ``switch_href`` points at the access gate (index.html) for Viewer <-> Team
    symmetry, relative to the page being rendered.
    """
    return (
        '<div class="footer"><div class="foot-l">Data from CraftED operations '
        '&middot; read-only mirror</div>'
        f'<div class="foot-r"><a href="{e(switch_href)}">Switch access</a></div></div>'
    )
...
def render_course_page(course, gen_amman):
    ...
    p.append(render_header(ts, brand_href="../../dashboard.html"))
    p.append('<div class="view">')
    # Hero
    p.append(
    ...
    p.append('</div>')
    p.append(render_footer(switch_href="../../index.html"))
    p.append('</div></body></html>')
    return "".join(p)
